...
{
    std::optional<int> width = settings["window"]["width"].value<int>();
    std::optional<int> height = settings["window"]["height"].value<int>();
    std::optional<int> cnkSize = settings["voxel_settings"]["chunk_size"].value<int>();
...
void Settings::save(bool showMessage)
{
...
Settings::Settings()
{
    if (std::filesystem::exists(engine::SETTINGS_FILE))
    {
        ...
        catch (const toml::parse_error &err)
        {
            ...
            std::cout << engine::console::warning() << "Resorting to default settings. Any changes you make to your settings this session will not be saved.\n";
            setDefault();
        }
    }
    else
...
Settings::~Settings()
{
    if (foundButBroken)
    {
        std::cout << engine::console::warning() << "Settings were not saved as we are using defaults due to a broken settings.toml.\n";
    }
}