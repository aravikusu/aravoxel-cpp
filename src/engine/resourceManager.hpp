...
public:
    ...
    /// @param vertexFile Name + extension of the vertex shader file.
    /// @param fragmentFile Name + extension of the fragment shader file.
    /// @param name The name that will be stored so you can retrieve it later.
    static Shader loadShader(const char *vertexFile, const char *fragmentFile, const std::string &name);
    ...
    static Texture2D loadTexture(const char *file, bool alpha, const std::string &name, bool flip = false);
