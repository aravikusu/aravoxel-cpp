...
public:
    ...
    // CALLBACKS BELOW \\
    // Has to be this way.
    static void frameBufferSize(int width, int height);
...
private:
    ...
    float lastX = static_cast<float>(settings.getHeight()) / 2.0f;
    float lastY = static_cast<float>(settings.getWidth()) / 2.0f;
    ...
    inline static auto frameBufferSizeCallback(GLFWwindow *window, const int width, const int height) -> void {
        auto *aravoxel = static_cast<Aravoxel *>(glfwGetWindowUserPointer(window));
        aravoxel->frameBufferSize(width, height);
    }

    inline static auto keyCallback(GLFWwindow *window, const int key, const int scancode, const int action,
                                   const int mods) {
        auto *aravoxel = static_cast<Aravoxel *>(glfwGetWindowUserPointer(window));
        aravoxel->glfwKey(key, scancode, action, mods);
    }

    inline static auto mouseCallback(GLFWwindow *window, const double xposIn, const double yposIn) {
        auto *aravoxel = static_cast<Aravoxel *>(glfwGetWindowUserPointer(window));
